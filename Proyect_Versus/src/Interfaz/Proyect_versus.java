...
public class Proyect_versus {
    ...
    private static void registrarUsuario(SistemaJuego sistema) {
        ...
        if (nombreUsuario == null || nombreUsuario.trim().isEmpty()) {
            JOptionPane.showMessageDialog(null, "El nombre de usuario no puede estar vacío. Intente nuevamente.", "Error",
                    JOptionPane.ERROR_MESSAGE);
            return; // Salir del método sin registrar
        }

        contrasenaUsuario = JOptionPane.showInputDialog(null, "Ingrese la contraseña que va a utilizar",
                "Registro contraseña", JOptionPane.QUESTION_MESSAGE);
        ...
        if (sistema.registrarUsuario(nuevoUsuario)) {
            JOptionPane.showMessageDialog(null, "Registro exitoso", "Registro finalizado",
    ...
    private static void mostrarMenu(Usuario usuario) {
        ...
        int seleccionLogueado = JOptionPane.showOptionDialog(null, "¿Qué desea realizar?", "Opciones",
                JOptionPane.DEFAULT_OPTION, 0, null, opcionesLogueado, opcionesLogueado[0]);
